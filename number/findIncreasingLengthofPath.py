...
class Solution(object):
    ...
    def neighbours(self, x, y):
        ...
        return [ (x,y) for x,y in candidates if x in range(self.leng) and y in range(self.leng) ]
            
    def longestIncreasingPath(self, matrix):
        self.matrix = matrix
        self.leng = len(matrix)
        max_length = 0

        for x, row in enumerate(matrix):
           for y, col in enumerate(row):
               current_length = self.findLength(x,y)
               if current_length>max_length:
                   max_length = current_length
        
        return max_length
                       
    def findLength(self, x, y):  
        max_length = 1
        neigbors = self.neighbours(x,y)

        for neigbor in neigbors:
            
            current_length = 0
               
            if self.matrix[neigbor[0]][neigbor[1]]>self.matrix[x][y]:
                current_length += 1 
                current_length += self.findLength(neigbor[0],neigbor[1])
...
def main():
    ...
    for test in tests:    
        ...
        try:
            ...
            calculated = Solution().longestIncreasingPath(data)
